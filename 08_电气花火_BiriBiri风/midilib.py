...
def drum_bar(dr, bar_start, style, rng, fill=False, vel_scale=1.0, bpb=4):
    ...
    if style == 'yo_intro':
        ...
        dr.hits(bar_start, [(0, 36, 72), (2, 36, 66)] +
                [(i * 0.5, 42, 40 if i % 2 == 0 else 30) for i in range(8)])
    elif style == 'bi_verse':
        dr.hits(bar_start, [(0, 36, 100), (1.5, 36, 84), (2.5, 36, 92),
                            (1, 39, 84), (3, 39, 86)])
        dr.hits(bar_start, [(i * 0.25, 42, (86 if i % 4 == 0 else 62) if i % 2 == 0 else 44)
                            for i in range(16)])
        if not fill:
            dr.hits(bar_start, [(3.5, 46, 70)])
    elif style == 'bi_chorus':
        dr.hits(bar_start, [(0, 36, 110), (1, 36, 100), (2, 36, 106), (3, 36, 102),
                            (1, 39, 102), (3, 39, 104)])
        dr.hits(bar_start, [(i * 0.5, 42, 88 if i % 2 == 0 else 64) for i in range(8)])
        dr.hits(bar_start, [(0.5, 46, 74), (1.5, 46, 74), (2.5, 46, 74), (3.5, 46, 74)])
        if fill:
            dr.hits(bar_start, [(3, 38, 104), (3.25, 38, 110), (3.5, 45, 116), (3.75, 43, 120)])
    elif style == 'bi_build':
        dr.hits(bar_start, [(i * 0.25, 42, min(118, 50 + i * 4)) for i in range(16)])
        dr.hits(bar_start, [(i * 0.25, 38, min(124, 58 + i * 5)) for i in range(8, 16)])
    elif style == 'bi_break':
        dr.hits(bar_start, [(0, 36, 88), (2, 38, 84), (1, 42, 42), (3, 42, 42),
                            (2.5, 42, 30), (3.5, 42, 30)])


def bass_offbeat8(trk, bar_start, chord, rng, vel=88):
    """四踩律动的反拍 8 分贝斯 (dance groove)"""
    r = chord.bass_midi(2)
    trk.n(bar_start, 0.4, r, hum(rng, vel - 12))
    for i in range(4):
        trk.n(bar_start + 0.5 + i, 0.34, r + (12 if i == 2 else 0), hum(rng, vel))


def synth_arp16(trk, bar_start, chord, rng, vel=52):
    """16 分琶音运动层 (副歌垫底律动)"""
    tones = chord.voice(low=64, high=84, n=4)
    pat = [i % len(tones) for i in (0, 1, 2, 3, 1, 2, 3, 2)]
    for i, idx in enumerate(pat):
        trk.n(bar_start + i * 0.25, 0.2, tones[idx], hum(rng, vel - (8 if i % 4 else 0), 3))
